    ...
    <section class="bg-red text-white" id="Images">
        ...
        <div class="row">
                <div class="col-lg-8 mx-auto text-center">
                    <h2 class="section-heading" style="font-size: 3em; font-family: 'Poppins', sans-serif;">Update Result</h2>
                    <hr class="light">
                </div>
            </div>
...
<?php
	...
	$id = $_POST["id"];
	$windspeed = $_POST["newwindspeed"];
	$DamageToProperty = $_POST["newDamageToProperty"];
	$year= $_POST["newyear"];
	$deaths= $_POST["newdeaths"];
	...
	foreach($Typhoons as $typhoon) 
	{
		...
		if($id == $oldid)
		{
			...
			$newNode = $xml->createElement("typhoon");
			$newNode->setAttribute("id",$id);
			$newNode->appendChild($xml->createElement("name",$name));
			$newNode->appendChild($xml->createElement("windspeed",$windspeed));
			$newNode->appendChild($xml->createElement("DamageToProperty",$DamageToProperty));
			$newNode->appendChild($xml->createElement("year",$year));
			$newNode->appendChild($xml->createElement("deaths",$deaths));
			
			$oldNode = $typhoon;
			
			$xml->getElementsByTagName("name")->item(0)->replaceChild($newNode,$oldNode);
			echo "Typhoon ID: $id<br>Typhoon Name: $name<br><br>";
			echo "Details updated...<br><center><a class='btn btn-black' href='update.php'>Back</a></center><br>";
			$xml->save("3KGR5.xml");
			break;
		}
	}	
	if($flag == 0) echo "Modification failed...<br><center><a class='btn btn-black' href='update.php'>Back</a></center><br>";
?>
